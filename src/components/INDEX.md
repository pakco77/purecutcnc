...
- `layout/` — Genmitsu task shell (three-row global/task/view header, one reusable `TaskDrawer`, contextual Inspector, and `taskShellModel` progress derivation), plus CAD toolbars, mode switching, and the shared `AppearanceControl` and `LanguageControl`; toolbar internals are documented in `layout/toolbar/INDEX.md`
